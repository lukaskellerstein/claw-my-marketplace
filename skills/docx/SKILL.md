...
4. **Color palette** — pick a palette matching the topic (see [references/design.md]({baseDir}/../docx/references/design.md))
5. **Font pairing** — pick header + body fonts (see [references/design.md]({baseDir}/../docx/references/design.md))

### Design Quality Target

Read the Design System in [references/design.md]({baseDir}/../docx/references/design.md). Key requirements:
...
Read [references/docx-js-api.md]({baseDir}/../docx/references/docx-js-api.md) for the full docx-js API reference.
...
```bash
python3 {baseDir}/../docx/scripts/thumbnail.py output.docx thumbnails
```
...
```bash
python3 {baseDir}/../_office-shared/scripts/office/validate.py output.docx -v
```
...
```bash
python3 {baseDir}/../docx/scripts/thumbnail.py input.docx thumbnails
```
...
|--------|---------|-------|
| `soffice.py` | LibreOffice integration (convert, env) | `python3 {baseDir}/../_office-shared/scripts/office/soffice.py input.docx output.pdf` |
| `validate.py` | XSD schema + structural validation | `python3 {baseDir}/../_office-shared/scripts/office/validate.py input.docx [-v] [--auto-repair] [--original orig.docx]` |
| `unpack.py` | Extract DOCX ZIP, pretty-print XML | `python3 {baseDir}/../_office-shared/scripts/office/unpack.py input.docx [output_dir] [--merge-runs] [--simplify-redlines]` |
| `pack.py` | Repack directory into DOCX ZIP | `python3 {baseDir}/../_office-shared/scripts/office/pack.py unpacked_dir [output.docx] [--validate]` |

### DOCX-Specific Scripts

| Script | Purpose | Usage |
|--------|---------|-------|
| `thumbnail.py` | DOCX → labeled page grid image | `python3 {baseDir}/../docx/scripts/thumbnail.py input.docx [output_prefix] [--cols N]` |
| `comment.py` | Add comments to unpacked DOCX | `python3 {baseDir}/../docx/scripts/comment.py unpacked_dir "text" --author "Name"` |
| `accept_changes.py` | Accept all tracked changes | `python3 {baseDir}/../docx/scripts/accept_changes.py input.docx [output.docx]` |

## Reference Files

| File | When to Read |
|------|-------------|
| [references/docx-js-api.md]({baseDir}/../docx/references/docx-js-api.md) | Always — full API reference for docx-js |
| [references/design.md]({baseDir}/../docx/references/design.md) | Always — document design system, colors, fonts, spacing |
