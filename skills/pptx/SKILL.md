...
Before choosing colors and fonts individually, **select a design template** from [references/templates.md]({baseDir}/../pptx/references/templates.md) that matches the presentation's purpose. The template provides a complete, pre-tested design system: palette, fonts, motif, rhythm, and background strategy.
...
2. **Layout type** — pick from the layout catalog (see [references/layouts.md]({baseDir}/../pptx/references/layouts.md))
...
4. **Colors + Fonts** — use the design template's recommendations, or customize from [references/design.md]({baseDir}/../pptx/references/design.md)
...
Read [references/design.md]({baseDir}/../pptx/references/design.md) for the full design system. Key requirements:
- **Follow your chosen design template** from [references/templates.md]({baseDir}/../pptx/references/templates.md) — don't pick colors/fonts/layouts independently
...
Read [references/pptxgenjs-api.md]({baseDir}/../pptx/references/pptxgenjs-api.md) for the full PptxGenJS API reference and [references/layouts.md]({baseDir}/../pptx/references/layouts.md) for slide layout implementations.
...
**Use the layout functions from [references/layouts.md]({baseDir}/../pptx/references/layouts.md)** — copy the relevant functions into your script and call them with your content.
...
```bash
python3 {baseDir}/../pptx/scripts/thumbnail.py output.pptx thumbnails
```
...
```bash
python3 {baseDir}/../_office-shared/scripts/office/validate.py output.pptx -v
```
...
```bash
python3 {baseDir}/../pptx/scripts/thumbnail.py input.pptx thumbnails/
```
...
See [references/templates.md]({baseDir}/../pptx/references/templates.md) for **7 topic-aware design templates** — complete presets for palette, fonts, motif, and rhythm.

See [references/design.md]({baseDir}/../pptx/references/design.md) for:
...
|--------|---------|-------|
| `soffice.py` | LibreOffice integration (convert, env) | `python3 {baseDir}/../_office-shared/scripts/office/soffice.py input.pptx output.pdf` |
| `validate.py` | XSD schema + structural validation | `python3 {baseDir}/../_office-shared/scripts/office/validate.py input.pptx [-v] [--auto-repair]` |
| `unpack.py` | Extract PPTX ZIP, pretty-print XML | `python3 {baseDir}/../_office-shared/scripts/office/unpack.py input.pptx [output_dir]` |
| `pack.py` | Repack directory into PPTX ZIP | `python3 {baseDir}/../_office-shared/scripts/office/pack.py unpacked_dir [output.pptx]` |

### PPTX-Specific Scripts

| Script | Purpose | Usage |
|--------|---------|-------|
| `thumbnail.py` | PPTX → labeled slide grid image | `python3 {baseDir}/../pptx/scripts/thumbnail.py input.pptx [output_prefix] [--cols N]` |
| `add_slide.py` | Duplicate a slide in unpacked PPTX | `python3 {baseDir}/../pptx/scripts/add_slide.py unpacked_dir slide_number` |
| `clean.py` | Remove unreferenced slides/media | `python3 {baseDir}/../pptx/scripts/clean.py unpacked_dir` |

## Reference Files

| File | When to Read |
|------|-------------|
| [references/templates.md]({baseDir}/../pptx/references/templates.md) | Always — 7 topic-aware design templates (palette, fonts, motif, rhythm) |
| [references/design.md]({baseDir}/../pptx/references/design.md) | Always — color palettes, dark themes, fonts, typography, motifs, depth techniques |
| [references/layouts.md]({baseDir}/../pptx/references/layouts.md) | Always — 14 slide layouts with PptxGenJS implementations |
| [references/pptxgenjs-api.md]({baseDir}/../pptx/references/pptxgenjs-api.md) | Always — full PptxGenJS API reference |
